...
```{r, results = "asis"}
postprint_url <- gsub(" ", replacement = "", x = pub_dat$postprint)
postprint_text <- ifelse(
  nchar(postprint_url) > 0,
  yes = paste0("[[postprint]](", postprint_url, ")"),
  no = "")
bib_years <- unlist(lapply(myBib, FUN = \(x) x$year))
for (yeari in as.character(2024:2019)) {
  cat(paste("\n###", yeari), "\n")
