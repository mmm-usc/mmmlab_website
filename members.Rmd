...
lines <- readLines("photo_titles_descriptions.txt")

photo_info <- strsplit(lines, "\\|", fixed = TRUE)
photo_titles <- sapply(photo_info, function(x) trimws(x[1]))
photo_descriptions <- sapply(photo_info, function(x) trimws(x[2]))

# generate filenames
photo_filenames <- gsub(" ", "_", photo_titles)  # replace spaces with underscores
photo_filenames <- tolower(photo_filenames)      # to lowercase
photo_filenames <- paste0(photo_filenames, ".jpg")  # all images are .jpg

img_path <-'static/images/slickr_files/'

# generate paths with alt text and descriptions
paths_w_alt_text <- mapply(function(filename, title, description) {
  tags$img(src = paste0(img_path, filename), title = description, alt = title, height = 400)
}, filename = photo_filenames, title = photo_titles, description = photo_descriptions, SIMPLIFY = FALSE)
